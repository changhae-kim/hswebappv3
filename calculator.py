...
compounds = {
        ...
        'C4'  : { 'H0':    489.       , 'Hsol/R': -1819., 'MW':   58.12  },
        'C5'  : { 'H0':    376.6074988, 'Hsol/R': -2211., 'MW':   72.15  },
        'C6'  : { 'H0':    176.       , 'Hsol/R': -2804., 'MW':   86.17  },
        'C7'  : { 'H0':    116.0282428, 'Hsol/R': -3279., 'MW':  100.20  },
        'C8'  : { 'H0':     72.7      , 'Hsol/R': -3724., 'MW':  114.22  },
        'C2H4': { 'H0':   2260.       , 'Hsol/R':  -661., 'MW':   28.05  },
...
class Calculator ():
    ...
    def __init__ ( self, temp=573.0, pressure=1.0 ):
        ...
        self.HsolR    = numpy.array([ value['Hsol/R'] for value in compounds.values() ])
        self.MW       = numpy.array([ value['MW'    ] for value in compounds.values() ])
        self.Hv       = self.get_henryslawconst()
        return

    def get_henryslawconst ( self, temp=None ):
        if temp is None:
            temp = self.temp
        return self.H0 * numpy.exp( self.HsolR * ( 1.0/temp - 1.0/573.0 ) )
    ...
    def get_gasphase ( self, w, temp=None, diction=False ):
        ...
        if temp is None:
            ...
            temp = temp
            Hv   = self.get_henryslawconst(temp)
